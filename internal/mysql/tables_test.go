...
func TestMySQLGetImportSelectQueryFor(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, "LOAD DATA FROM S3 FILE 'S3-ap-southeast-4://path/to/bucket/table_name.csv' REPLACE INTO TABLE `table_name` FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\\n'", query)
